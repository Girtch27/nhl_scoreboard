...
NHL_TEAM_LOGO_URL = "#https://www-league.nhlstatic.com/images/logos/teams-current-primary-light/"
...
def get_team_arena_name(team_name):
    ...
    raise Exception("Could not find arena name for team {0}".format(team_name))

def get_team_arena_city(team_name):
    ...
    raise Exception("Could not find arena city for team {0}".format(team_name))
...
def get_team_history(team_name):
    ...
    raise Exception("Could not find history info for team {0}".format(team_name))

def get_team_division_info(team_name):
    ...
    raise Exception("Could not find division info for team {0}".format(team_name))

def get_team_conference_info(team_name):
    ...
    raise Exception("Could not find conference info for team {0}".format(team_name))
...
def get_next_game_date2(team_id):
    "function to get the time of the next game as regular time like 7:00pm"
    date_test = datetime.date.today()
    gameday = check_game_status(team_id, date_test)
    ...
    utc_game_time = requests.get(url).json()
    utc_game_time = utc_game_time['dates'][0]['games'][0]['gameDate']
    next_game_time = convert_to_local_time(utc_game_time) - datetime.timedelta(hours=12)

    return next_game_time

def get_next_game_info2(team_id):
    "get the team names of the next game"
    date_test = datetime.date.today()
    gameday = check_game_status(team_id, date_test)
